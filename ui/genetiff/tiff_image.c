...
{ int i, area, bps, delta;
  ...
  { delta = scale - channel->scale;
    ...
    { unsigned char *source = (unsigned char *) (channel->plane); 
      if (bps == 2)
    ...
    { unsigned short *source = (unsigned short *) (channel->plane);
      unsigned int   *target = (unsigned int   *) (channel->plane);
      for (i = area-1; i >= 0; i--)
        target[i] = source[i];
    }
  ...
  { delta = channel->scale - scale;
    ...
    { if (channel->bytes_per_pixel == 4)
      { unsigned int *source = (unsigned int *) (channel->plane); 
        if (bps == 2)
        { unsigned short *target = (unsigned short *) (channel->plane);
          for (i = 0; i < area; i++)
            target[i] = source[i];
