...
#if 0
int Save_Whiskers( char *filename, Whisker_Seg *wv, int n )
{ printf("Saving %d whiskers\n",n);
  if(n)
  { /* Open file */
...
{ Index *idx = (Index*) Guarded_Malloc( sizeof(Index), "Build whisker index");
  int i,nframes;

  // sort in ascending (frameid, whiskerid) order
  qsort( wv, n, sizeof(Whisker_Seg), &Compare_Whisker_Seg_Index );
  nframes = wv[n-1].time+1;

  // build the index
  { Whisker_Seg **index = (Whisker_Seg**) Guarded_Malloc( sizeof(Whisker_Seg*) * nframes, "Build whisker index" );
    int *sz = (int*) Guarded_Malloc( sizeof(int) * nframes, "Build whisker index" );
...
Whisker_Seg *read_segments_old( FILE *file, int *n)
{ Whisker_Seg *wv;
  int nwhiskers = 0;
  size_t nch;
...
#if 0
Whisker_Seg *Load_Whiskers_From_File( FILE *file, int *n)
{ char format[33];
  char format_whisker1[] = "whisker1";

  rewind(file);
  if( is_file_whiskbin1( file ) )
    return read_segments_whiskbin1(file,n);
  else
  { fscanf( file, "%32s", format );
    if( strncmp( format, format_whisker1, sizeof(format_whisker1) ) == 0 )
      return read_segments_whisker1( file, n);
    else
      return read_segments_old( file, n);
  }

  return (NULL);
}

Whisker_Seg *Load_Whiskers( char *filename, int *n )
{ FILE *file = fopen(filename,"r");
  Whisker_Seg *wv;
  if( !file ) goto error;
  wv = Load_Whiskers_From_File( file, n);
  fclose(file);
  return wv;
error:
